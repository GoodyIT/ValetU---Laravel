 ...
 * COS(abs(t1.latitude) * pi()/180) * POWER(SIN(($lng - t1.longitude) * pi()/180 / 2),2) )) < 16 ";
        ...
        foreach ($parkinglots as $lotkey => $parkinglot) {
            ...
            $sql = "SELECT t1.id, t1.star, t1.photourl, t1.review, t1.updated_at, t2.id as userId, t2.name, t2.email, t2.uber_credential as profileId FROM trips as t1 join uberusers as t2 on t1.user_id = t2.id where t1.parkinglot_id = $parkinglot->id and t1.review != ''";
    ...
    public function savereview(Request $request)
    {
        ...
        if (!isset($users->id) || $file->getClientOriginalName() == "") {
            ...
            try {
              ...
              $parkinglot = Parkinglot::find($parkinglot_id);
              $parkinglot->star = $parkinglot->star + intval($star);
              $parkinglot->save();
